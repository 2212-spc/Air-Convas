...
class Button:
    ...
    def render(self, frame: np.ndarray, is_confirmed: bool = False):
        """渲染按钮"""
        # 背景色
        if self.active:
            color = (100, 200, 100)  # 绿色（激活/已选中）
        elif self.hovered and not is_confirmed:
            color = (200, 200, 200)  # 亮灰（悬停，仅未确认时）
        elif is_confirmed:
            color = (120, 180, 120)  # 浅绿（已确认但未激活）
        else:
        ...
        # 边框（已确认的按钮边框变化）
        if is_confirmed:
            border_color = (100, 255, 100) if self.hovered else (80, 200, 80)
        else:
...
class ColorButton:
    ...
    def render(self, frame: np.ndarray, is_confirmed: bool = False):
        ...
        if self.active:
            ...
            cv2.circle(frame, (cx, cy), radius + 2, (100, 255, 100), 2, lineType=cv2.LINE_AA)
        elif self.hovered:
...
class ParticleModeUI:
    ...
    def __init__(self, width: int, height: int):
        ...
        # 已确认的按钮（不再显示转圈动画）
        self.confirmed_buttons = set()  # 存储已确认按钮的索引
        
        # 悬停自动确认机制
        self.hover_frames = 0
        self.last_hover_item = None
        self.dwell_frames = 30  # 悬停30帧（约1秒）自动确认
    ...
    def handle_mouse(self, event: int, x: int, y: int, flags, param):
        ...
        if event == cv2.EVENT_MOUSEMOVE:
            ...
            # 悬停计时（用于自动确认）
            current_hover = None
            # 检查当前悬停的按钮（只对模型和颜色按钮启用悬停确认）
            for i, btn in enumerate(self.buttons):
                if btn.hovered and i < 3:  # 只对模型按钮启用悬停确认
                    current_hover = ('model', i)
                    break
            if current_hover is None:
                for i, btn in enumerate(self.color_buttons):
                    if btn.hovered:
                        current_hover = ('color', i)
                        break
            
            # 更新悬停计数
            if current_hover == self.last_hover_item and current_hover is not None:
                # 如果已确认过，不再计数，且重置进度
                if current_hover not in self.confirmed_buttons:
                    self.hover_frames += 1
                    # 达到阈值，自动确认
                    if self.hover_frames >= self.dwell_frames:
                        self._auto_confirm_hover(current_hover)
                        self.hover_frames = 0
                else:
                    # 已确认的按钮，进度归零
                    self.hover_frames = 0
                ...
                self.hover_frames = 0
                self.last_hover_item = current_hover
            ...
            for i, btn in enumerate(self.buttons):
                if btn.contains(x, y):
                    ...
                    # 如果是模型按钮，标记为已确认
                    if i < 3:  # 前3个是模型按钮
                        self.confirmed_buttons.add(('model', i))
                    # 清除悬停状态，防止重复计数
                    self.hover_frames = 0
                    self.last_hover_item = None
                    return
            ...
            for i, btn in enumerate(self.color_buttons):
                if btn.contains(x, y):
                    ...
                    btn.active = True
                    # 标记颜色按钮为已确认
                    self.confirmed_buttons.add(('color', i))
                    # 清除悬停状态，防止重复计数
                    self.hover_frames = 0
                    self.last_hover_item = None
                    return

    def _auto_confirm_hover(self, item_key):
        """悬停自动确认"""
        item_type, item_index = item_key
        
        if item_type == 'model':
            # 模型按钮
            btn = self.buttons[item_index]
            btn.action()
            self.confirmed_buttons.add(item_key)
        elif item_type == 'color':
            # 颜色按钮
            btn = self.color_buttons[item_index]
            if self.on_color_change:
                self.on_color_change(btn.color)
            # 更新激活状态
            for cb in self.color_buttons:
    ...
    def render(self, frame: np.ndarray) -> np.ndarray:
        ...
        for i, btn in enumerate(self.buttons):
            ...
            # 检查是否已确认（前3个是模型按钮）
            is_confirmed = ('model', i) in self.confirmed_buttons if i < 3 else False
            btn_copy.render(output, is_confirmed)
            
            # 悬停进度条（仅在悬停且未确认时显示）
            if btn.hovered and not is_confirmed and i < 3 and self.last_hover_item == ('model', i):
                progress = self.hover_frames / self.dwell_frames
                self._render_hover_progress(output, btn.x + panel_offset, btn.y, btn.w, btn.h, progress)
        ...
        for i, btn in enumerate(self.color_buttons):
            ...
            btn_copy.active = btn.active
            # 检查颜色按钮是否已确认
            is_confirmed = ('color', i) in self.confirmed_buttons
            btn_copy.render(output, is_confirmed)
            
            # 悬停进度圈（仅在悬停且未确认时显示）
            if btn.hovered and not is_confirmed and self.last_hover_item == ('color', i):
                progress = self.hover_frames / self.dwell_frames
                self._render_hover_circle_progress(output, btn.x + panel_offset + btn.size // 2, 
                                                   btn.y + btn.size // 2, btn.size // 2 + 5, progress)
    ...
    def _render_hover_progress(self, frame: np.ndarray, x: int, y: int, w: int, h: int, progress: float):
        """渲染悬停进度条（矩形按钮）"""
        bar_height = 3
        bar_y = y + h + 5
        ...
        # 进度条背景
        cv2.rectangle(frame, (x, bar_y), (x + w, bar_y + bar_height), (80, 80, 80), -1)
        # 进度条前景
        cv2.rectangle(frame, (x, bar_y), (x + bar_width, bar_y + bar_height), (100, 255, 100), -1)
    
    def _render_hover_circle_progress(self, frame: np.ndarray, cx: int, cy: int, radius: int, progress: float):
        """渲染悬停进度圈（圆形按钮）"""
        # 绘制进度弧线
        angle = int(360 * progress)
        if angle > 0:
            # OpenCV的椭圆函数：从-90度（顶部）开始，顺时针
            cv2.ellipse(frame, (cx, cy), (radius, radius), -90, 0, angle, 
                       (100, 255, 100), 3, lineType=cv2.LINE_AA)
