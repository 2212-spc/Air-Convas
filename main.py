...
def main() -> None:
    ...
    while True:
        ...
        if key == ord('x') and APP_MODE == "PPT":
            ...
            print("PPT: 透明叠加层已清空")
        # enrich 分支的额外功能：在交互特效开启时，'5' 键可以切换下一个效果
        if key == ord('5') and ENABLE_INTERACTIVE_EFFECTS and not particle_mode_manager.is_active:
            interactive_effects.next_effect()
        if key == ord('w'):
